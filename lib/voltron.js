...
function installDevDeps(voltronExtensions, directory) {
  ...
  return co(function* () {
    for (let ext of voltronExtensions) {
      ...
      yield npminstall({
        root: extPath,
        pkgs: Object.keys(pkg.devDependencies).map(name => ({ name, version: pkg.devDependencies[name] }))
      });
    }
  }).catch(_ => installDevDeps(voltronExtensions, directory));
}
 ...
 *
 * @param {Array<Object>} configs - All extension configs
 * @param {String} outputDir - Where the result of the build should go
 *
